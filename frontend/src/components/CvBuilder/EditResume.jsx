...
import { FaRobot, FaGraduationCap, FaBriefcase, FaTools, FaPlus } from "react-icons/fa";
...
const EditResume = () => {
  ...
  const params = useParams(); // سيقوم بقراءة :userID و :resumeTitle من الرابط
  ...
  // 2. جلب بيانات السيرة الذاتية القديمة بناءً على الرابط لتعبئة الحقول
  useEffect(() => {
    if (params.userID && params.resumeTitle) {
      const fetchResumeData = async () => {
        try {
          ...
          const cvData = res.data.resume;
          
          if (cvData) {
            setResumeTitle(cvData.resumeTitle || params.resumeTitle);
          ...
          console.error(err);
          toast.error("فشل في تحميل بيانات السيرة الذاتية للتعديل");
        }
      };
      fetchResumeData();
    }
  ...
  const imporveSummary = async () => {
    try {
      ...
      if (data.error === false) {
        setPersonalInfo(prev => ({ ...prev, summary: data.text }));
      }
      setWait(false);
    } catch (error) {
      console.log(error);
  ...
  const imporveJob = async () => {
    try {
      ...
      if (data.error === false) {
        setCurrentExp(prev => ({ ...prev, description: data.text }));
      }
      setWaitJob(false);
    } catch (error) {
      console.log(error);
  ...
  const handleAddExperience = () => { 
    if (!currentExp.jobName) return;
    setExperience([...experience, currentExp]); 
    setCurrentExp({ jobName: '', companyName: '', companyAddress: '', startDate: '', endDate: '', stillWorking: false, description: '' }); 
  };
  
  const handleAddEducation = () => { 
    if (!currentEdu.faculty) return;
    setEducation([...education, currentEdu]); 
    setCurrentEdu({ faculty: '', universityName: '', address: '', grade: '', startDate: '', endDate: '' }); 
  };
  
  const handleAddSkill = () => { 
    if (!currentSkill.name) return;
    setSkills([...skills, currentSkill]); 
    setCurrentSkill({ name: '', type: '', level: '' }); 
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await axios.put(`${apiUrl}/resume/${params.userID}/${params.resumeTitle}`, resumeData, { withCredentials: true });
      toast.success(res.data.message || "تم تحديث السيرة الذاتية بنجاح");
    ...
    <div className='mt-32 px-4 md:px-10 max-w-6xl mx-auto mb-20' dir='rtl'>
      ...
      <section className='mt-12 text-right'>
        ...
        <input 
          ...
          className='border-2 border-gray-100 rounded-xl px-4 py-2 focus:border-blue-500 w-full mb-8' 
        />
        ...
        <div className='p-8 space-y-6 bg-white rounded-2xl shadow-sm border border-gray-100'>
          ...
          <button
            ...
            className="relative flex items-center gap-2 px-6 py-3 rounded-xl text-white font-semibold bg-gradient-to-r from-blue-600 via-blue-500 to-cyan-400 shadow-lg shadow-blue-500/40 hover:shadow-cyan-400/60 hover:scale-105 transition-all duration-300 overflow-hidden"
          >
            ...
            <span className="relative">{wait ? "يتم التحميل" : "تحسين بالذكاء الاصطناعي"}</span>
          </button>
      ...
      <section className='mt-12 text-right'>
        ...
        <div className='p-8 space-y-6 bg-white rounded-2xl shadow-sm border border-gray-100'>
          ...
          >
            <span className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent translate-x-[-100%] hover:translate-x-[100%] transition-transform duration-700"></span>
            <FaRobot className="relative text-lg animate-pulse" />
            <span className="relative">{waitJob ? "يتم التحميل" : "تحسين بالذكاء الاصطناعي"}</span>
          </button>
      ...
      <section className='mt-12 text-right'>
        ...
        <div className='p-8 space-y-6 bg-white rounded-2xl shadow-sm border border-gray-100'>
          ...
          {education.map((edu, idx) => (
            <div key={idx} className='flex justify-between items-center p-4 border border-gray-200 rounded-xl bg-gray-50 mb-2'>
              <span>{edu.faculty} - {edu.universityName}</span>
              <button onClick={() => removeItem(idx, 'edu')} className="text-red-500 hover:text-red-700">حذف</button>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className='mt-12 text-right'>
        ...
        <div className='p-8 bg-white rounded-2xl shadow-sm border border-gray-100'>
          ...
          {skills.map((skill, idx) => (
            <div key={idx} className='flex justify-between items-center p-4 border border-gray-200 rounded-xl bg-gray-50 mt-4'>
              <span>{skill.name} - {skill.type} - {skill.level}</span>
              <button onClick={() => removeItem(idx, 'skill')} className="text-red-500 hover:text-red-700">حذف</button>
            </div>
          ))}
        </div>
      </section>

      {/* زر تحديث السيرة الذاتية */}
      <div className='flex justify-end gap-4 mt-12'>
        <button onClick={handleUpdate} className='bg-green-600 text-white font-bold py-3 px-12 rounded-xl shadow-lg hover:bg-green-700 transition-all'>
